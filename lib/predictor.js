...
export async function runPrediction(client, { rank, category, course, round, year, includeFullSectionA = false }) {
  ...
  // Determine effective year: explicit > latest year present for (category, round)
  let effectiveYear = year != null ? Number(year) : null
  if (!effectiveYear) {
    const yRes = await client
      .from('cutoffs')
      .select('year')
      ...
      .limit(1)
    if (!yRes.error && yRes.data && yRes.data.length) effectiveYear = Number(yRes.data[0].year)
  }
  ...
  const { data: cutoffs, error: cutErr } = await q
