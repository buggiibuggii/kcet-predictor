...
export const CATEGORIES = ['GM', 'GMK', '1G', '2AG', '2BG', '3AG', '3BG', 'SCG', 'STG']
export const ROUNDS = ['R1', 'R2', 'Extended']

// Base cutoff anchors per (college tier, course) for GM round R1.
// We then derive cutoffs for other categories/rounds by multipliers.
function baseCutoff(tier, courseCode) {
  // Lower closing_rank = more competitive
  const courseFactor = {
...
function categoryMultiplier(cat) {
  return {
    GM: 1.0, GMK: 1.05, '1G': 1.4, '2AG': 1.3, '2BG': 1.5,
    '3AG': 1.6, '3BG': 1.5, SCG: 2.2, STG: 2.8,
  }[cat] || 1.0
}
